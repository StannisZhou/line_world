...
class CoarseLayer(Component):
    ...
    def params_proc(params):
        ...
        params['brick_self_rooting_prob'] = torch.cat((
            torch.ones(1), torch.zeros(params['n_templates'])
        ))
    ...
    def get_no_parents_prob(self, coarse_state, layer_list, aggregate=True):
        ...
        no_parents_prob = lo.get_no_parents_prob(
            ...
            top_layer.grid_size, self.n_templates, bottom_layer.n_channels,
    ...
    def get_connections(self, coarse_state, layer_list):
        ...
        on_bricks_prob = lo.get_on_bricks_prob(coarse_state)
    ...
    def draw_sample(self, state):
        no_parents_prob = 1 - lo.get_on_bricks_prob(state)
        lo.validate_no_parents_prob(no_parents_prob, True, state.shape[:3])
        n_templates = self.params['templates'].shape[0]
        state_shape = state.shape[:3] + torch.Size([n_templates + 1])
        prob = torch.zeros(state_shape)
    ...
    def get_log_prob(self, state, coarse_state):
        no_parents_prob = 1 - lo.get_on_bricks_prob(state)
        lo.validate_no_parents_prob(no_parents_prob, False, state.shape[:3])
